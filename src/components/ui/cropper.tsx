...
const CropperRoot: React.FC<CropperRootProps> = ({
  ...
  const getPinchDistance = (touches: TouchList): number =>
    Math.sqrt(Math.pow(touches[1].clientX - touches[0].clientX, 2) + Math.pow(touches[1].clientY - touches[0].clientY, 2))
...
const CropperImagePrimitive: React.FC<
...
const CropperCropAreaPrimitive: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ className, style, ...restProps }) => {
  ...
  return <div {...areaProps} style={{ ...areaProps.style, ...style, ...areaStyle }} className={className} {...restProps}></div>
}

const CropperDescriptionPrimitive: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  children,
  className,
  ...restProps
}) => {
...
function Cropper({ className, ...props }: React.ComponentProps<typeof CropperRoot>) {
  return (
    <CropperRoot
      className={cn(
...
function CropperDescription({ className, ...props }: React.ComponentProps<typeof CropperDescriptionPrimitive>) {
  return <CropperDescriptionPrimitive className={cn("sr-only", className)} data-slot="cropper-description" {...props} />
}

function CropperImage({ className, ...props }: React.ComponentProps<typeof CropperImagePrimitive>) {
  return (
    <CropperImagePrimitive
...
function CropperCropArea({ className, ...props }: React.ComponentProps<typeof CropperCropAreaPrimitive>) {
  return (
    <CropperCropAreaPrimitive
      className={cn(
